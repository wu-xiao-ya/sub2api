...
func (r *usageLogRepository) GetAccountWindowStatsBatch(ctx context.Context, accountIDs []int64, startTime time.Time) (map[int64]*usagestats.AccountStats, error) {
	...
	query := fmt.Sprintf(`
		SELECT
			...
			COALESCE(SUM(ul.actual_cost), 0) as user_cost
		FROM usage_logs ul
		WHERE ul.account_id = ANY($1) AND ul.created_at >= $2
...
func (r *usageLogRepository) GetStatsWithFilters(ctx context.Context, filters UsageLogFilters) (*UsageStats, error) {
	...
	conditions = appendUsageSourceWhereCondition(conditions, filters, "")
	...
	query := fmt.Sprintf(`
		...
		FROM usage_logs ul
		%s
	`, inlineImageAwareAccountCostExpr("ul"), buildWhere(conditions))
	...
	runEndpoints := func(c context.Context) {
		res, err := r.getEndpointStatsByColumnWithFilters(c, "inbound_endpoint", start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.ModelFilterSource, filters.RequestType, filters.Stream, filters.BillingType, filters.BillingMode, filters.IncludeMonitorUsage)
		if err != nil {
	...
	runUpstream := func(c context.Context) {
		res, err := r.getEndpointStatsByColumnWithFilters(c, "upstream_endpoint", start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.ModelFilterSource, filters.RequestType, filters.Stream, filters.BillingType, filters.BillingMode, filters.IncludeMonitorUsage)
		if err != nil {
			if !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
	...
	runPaths := func(c context.Context) {
		res, err := r.getEndpointPathStatsWithFilters(c, start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.ModelFilterSource, filters.RequestType, filters.Stream, filters.BillingType, filters.BillingMode, filters.IncludeMonitorUsage)
		if err != nil {
			if !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
...
func (r *usageLogRepository) getEndpointStatsByColumnWithFilters(ctx context.Context, endpointColumn string, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, modelSource string, requestType *int16, stream *bool, billingType *int8, billingMode string, includeMonitorUsage bool) (results []EndpointStat, err error) {
	...
	if accountID > 0 && userID == 0 && apiKeyID == 0 {
		// Account-scoped callers read the account-side cost; image rows must
		// bill per generated image rather than per token.
		actualCostExpr = fmt.Sprintf("COALESCE(SUM(%s), 0) as actual_cost", inlineImageAwareAccountCostExpr("ul"))
	}
...
func (r *usageLogRepository) getEndpointPathStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, modelSource string, requestType *int16, stream *bool, billingType *int8, billingMode string, includeMonitorUsage bool) (results []EndpointStat, err error) {
	actualCostExpr := "COALESCE(SUM(ul.actual_cost), 0) as actual_cost"
	if accountID > 0 && userID == 0 && apiKeyID == 0 {
		// Account-scoped callers read the account-side cost; image rows must
		// bill per generated image rather than per token.
