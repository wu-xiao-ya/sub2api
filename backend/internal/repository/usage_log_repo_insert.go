...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	usageSource := nullString(log.UsageSource)
	latencyBreakdown := nullUsageLatencyBreakdownJSON(log.LatencyBreakdown)
	promotionID := nullInt64(log.PromotionID)
	promotionName := nullString(log.PromotionName)
	baseRateMultiplier := log.BaseRateMultiplier
