...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		// 发送请求
		resp, err = s.httpUpstream.DoWithTLS(WithAccountOutbound(upstreamReq, account, proxyURL), proxyURL, account.ID, account.Concurrency, tlsProfile)
		if err != nil {
		...
		if resp.StatusCode == 400 {
			...
			if readErr == nil {
				...
				if s.shouldRectifySignatureError(ctx, account, respBody, reqModel) {
					...
					releaseRetryCtx()
					if buildErr == nil {
						retryResp, retryErr := s.httpUpstream.DoWithTLS(WithAccountOutbound(retryReq, account, proxyURL), proxyURL, account.ID, account.Concurrency, tlsProfile)
						if retryErr == nil {
							...
							if retryReadErr == nil && retryResp.StatusCode == 400 && s.isSignatureErrorPattern(ctx, account, retryRespBody) {
								...
								if looksLikeToolSignatureError(msg2) && time.Since(retryStart) < maxRetryElapsed {
									...
									releaseRetryCtx2()
									if buildErr2 == nil {
										retryResp2, retryErr2 := s.httpUpstream.DoWithTLS(WithAccountOutbound(retryReq2, account, proxyURL), proxyURL, account.ID, account.Concurrency, tlsProfile)
										if retryErr2 == nil {
				...
				if isThinkingBudgetConstraintError(errMsg) && s.settingService.IsBudgetRectifierEnabled(ctx) {
					...
					if applied && time.Since(retryStart) < maxRetryElapsed {
						...
						if buildErr == nil {
							budgetRetryResp, retryErr := s.httpUpstream.DoWithTLS(WithAccountOutbound(budgetRetryReq, account, proxyURL), proxyURL, account.ID, account.Concurrency, tlsProfile)
							if retryErr == nil {
