...
func (s *EmailService) SendEmailWithConfig(config *SMTPConfig, to, subject, body string) error {
	...
	// Email is sent successfully after w.Close(); some servers have
	// non-standard QUIT responses, so do not turn those into a false failure.
	_ = client.Quit()
	return nil
}

// smtpTestRootCAs is only populated by focused SMTP tests.
...
// connectSMTP is shared by send and test-connection. UseTLS first attempts
// implicit TLS, then retries as mandatory STARTTLS when the remote endpoint is
// a submission port that begins with a plaintext SMTP greeting.
...
// TestSMTPConnectionWithConfig uses the same TLS and STARTTLS negotiation as
// actual delivery so a successful test reflects send-time behavior.
