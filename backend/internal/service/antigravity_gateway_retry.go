...
func (s *AntigravityGatewayService) handleSmartRetry(p antigravityRetryLoopParams, resp *http.Response, respBody []byte, baseURL string, urlIdx int, availableURLs []string) *smartRetryResult {
	...
	if shouldSmartRetry {
		...
		for attempt := 1; attempt <= maxAttempts; attempt++ {
			...
			retryResp, retryErr := p.httpUpstream.Do(WithAccountOutbound(retryReq, p.account, p.proxyURL), p.proxyURL, p.account.ID, p.account.Concurrency)
...
urlFallbackLoop:
	for urlIdx, baseURL := range availableURLs {
		...
		for attempt := 1; attempt <= antigravityMaxRetries; attempt++ {
			...
			resp, err = p.httpUpstream.Do(WithAccountOutbound(upstreamReq, p.account, p.proxyURL), p.proxyURL, p.account.ID, p.account.Concurrency)
			if err == nil && resp == nil {
