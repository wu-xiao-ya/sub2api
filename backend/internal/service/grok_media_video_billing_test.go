...
func TestIsGrokVideoStatusBillable(t *testing.T) {
	...
	require.True(t, IsGrokVideoStatusBillable([]byte(`{"status":"done","video":{"url":"https://vidgen.x.ai/x.mp4"}}`)))
}

func TestExtractGrokVideoBillingFromStatusBodyUsesStatusDurationAndPendingResolution(t *testing.T) {
	t.Parallel()
...
func TestGrokMediaUsageFromResponseVideoStatusBillsOnURL(t *testing.T) {
	...
	meta := grokMediaUsageFromResponse(
		...
		[]byte(`{"status":"done","video":{"url":"https://vidgen.x.ai/a.mp4","duration":9,"resolution":"480p"}}`),
	)
	require.Equal(t, 1, meta.VideoCount)
	require.Equal(t, 9, meta.VideoDurationSeconds)
	require.Empty(t, meta.VideoResolution)
