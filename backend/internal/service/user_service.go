...
type UserListFilters struct {
	...
	IsMonitoringUser *bool  // Filter the internal station-monitoring users.
...
type UserRepository interface {
	...
	// GetByIDIncludeDeleted ???????? ID ????????????????/usage ?????
	...
	// AddGroupToAllowedGroups ????????????? allowed_groups?????????
...
// TouchLastActive ?????? users.last_active_at????????????
// ??????????????????
...
// TouchLastActiveForUser ???????????? last_active_at???????????
...
func (s *UserService) sendNotifyVerifyEmail(ctx context.Context, emailService *EmailService, userID int64, email, code, locale string) error {
	...
	subject := fmt.Sprintf("[%s] ??????? / Notification Email Verification", siteName)
...
<body>
    <div class="container">
        ...
        <div class="content">
            <p style="font-size: 18px; color: #333;">??????? / Notification Email Verification</p>
