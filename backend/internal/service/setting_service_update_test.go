...
func TestSettingService_UpdateSettings_DefaultSubscriptionsAreReadOnly(t *testing.T) {
	repo := &settingUpdateRepoStub{}
	groupReader := &defaultSubGroupReaderStub{
		errBy: map[int64]error{13: ErrGroupNotFound},
	}
	svc := NewSettingService(repo, &config.Config{})
	...
	err := svc.UpdateSettings(context.Background(), &SystemSettings{
		DefaultSubscriptions: []DefaultSubscriptionSetting{
			...
			{GroupID: 13, ValidityDays: 30},
		},
	})
	require.NoError(t, err)
	require.Empty(t, groupReader.calls)
	_, written := repo.updates[SettingKeyDefaultSubscriptions]
