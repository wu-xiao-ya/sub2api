...
func classifySelectionFailureError(err error, fallback noAccountErrorClassification) noAccountErrorClassification {
	...
	// to a rate-limit verdict. The availability diagnosis already established
	// that no persistently eligible account can serve this model; a transient
	// cooldown on one candidate does not change that fact.
