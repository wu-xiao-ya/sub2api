...
func (Group) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Float("contributor_reward_multiplier").
			SchemaType(map[string]string{dialect.Postgres: "decimal(10,4)"}).
			Default(0).
			Comment("Contribution reward multiplier; 0 disables rewards for this group"),
		...
		field.Bool("force_openai_fast").
			Default(false).
			Comment("Force OpenAI or composite groups to use service_tier=priority"),
		field.Bool("free_openai_fast").
			Default(false).
			Comment("Bill Fast requests in OpenAI or composite groups at Standard pricing"),
...
func (Group) Edges() []ent.Edge {
	return []ent.Edge{
		...
		edge.To("subscription_plan_groups", SubscriptionPlanGroup.Type),
