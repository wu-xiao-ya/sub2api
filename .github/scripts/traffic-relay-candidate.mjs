...
export async function verifyTrafficRelay({ api, admin, base }) {
  ...
  try {
    ...
    assert(initial.attempts >= 1, 'Opted-in account did not use relay')
    assert.equal((await stats(18094)).attempts, 0, 'Healthy relay used fallback')
    ...
    assert(fallback.attempts >= 1, 'Relay failure did not use account proxy')
