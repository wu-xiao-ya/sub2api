...
describe('user UsageView', () => {
  ...
  it('exports csv with current filters and without admin-only fields', async () => {
    ...
    expect(csvContent.slice(1)).toBe([
      'Time,API Key Name,Model,Reasoning Effort,Inbound Endpoint,IP Address,Type,admin.usage.billingSource,Billing Mode,Input Tokens,Output Tokens,Cache Read Tokens,Cache Creation Tokens,usage.promotion,usage.promotionBaseRate,usage.promotionFinalRate,usage.promotionPricePercent,Rate Multiplier,Billed Cost,Original Cost,Latency version,First response (ms),First event (ms),First output (ms),First text (ms),Stage total (ms),First Token (ms),Duration (ms),Generation (ms),Output Speed (Token/s)',
      '2026-03-08T00:00:00Z,demo-key,gpt-5.4,"\'-",,203.0.113.10,Sync,,Token,4057,101,278272,4,,,1,,1,0.09288300,0.09288300,1,,,,,345,12,345,333,303.3',
    ].join('\n'))
