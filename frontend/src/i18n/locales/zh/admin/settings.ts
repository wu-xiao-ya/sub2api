    ...
    settings: {
      ...
      features: {
        channelMonitor: {
          ...
          mode: '默认监控视图',
          modeHint: '只影响用户打开渠道监控页时的默认视图。主动诊断与真实流量趋势都会继续运行。',
          modeV2: '真实流量趋势',
          modeV1: '实时诊断',
          modeV2Hint: '默认展示来自真实用户请求的成功率、首 Token、缓存率和趋势。',
          modeV1Hint: '默认展示站内主动探测、账号来源和图片检测结果。',
          defaultInterval: '默认检测间隔（秒）',
          defaultIntervalHint: '新建渠道监控时表单的默认值，可被单个渠道覆盖。范围 15 – 3600 秒。',
          hideThroughput: '对用户隐藏吞吐速率（RPM / TPM）',
          hideThroughputHint: '管理员仍可见完整指标；错误率、延迟、缓存率和状态不受影响。',
          accountProbe: {
            title: '自适应账号探测',
            description: '对绑定账号组的监控项按候选账号状态执行自适应探测。',
            enabled: '启用自适应账号探测',
            enabledHint: '关闭后保留原有渠道检测路径，不再按账号组候选状态扩展探测。',
            confirmAttempts: '确认次数',
            confirmAttemptsHint: '首次结果异常后追加确认探测的次数。范围 0 – 2。',
            degradedThreshold: '正常延迟阈值（ms）',
            degradedThresholdHint: '响应超过该阈值时标记为延迟较高。范围 100 – 120000 ms。',
            maxCandidates: '候选账号数',
            maxCandidatesHint: '每次最多参与自适应探测的候选账号数。范围 1 – 20。',
            parallelism: '并发探测数',
            parallelismHint: '每次最多同时探测的账号数。范围 1 – 20。',
      ...
      site: {
        ...
        apiEndpointProbeInterval: '端点延迟检测间隔',
        apiEndpointProbeIntervalHint: 'API Keys 页面自动检测 API 端点延迟的间隔秒数。0 表示关闭自动检测，仅手动刷新；非 0 会按 3-300 秒保存。',
