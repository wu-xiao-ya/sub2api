export default {
    ...
    channelMonitor: {
      ...
      runNow: '立即检测',
      viewImage: '查看图片',
      viewLatestImage: '查看上次生成的图片',
      latestImageTitle: '{name} 的上次生图',
      latestImageAlt: '渠道监控上次生成的图片',
      noLatestImage: '还没有成功生成过图片',
      imageLoadFailed: '加载上次生成的图片失败',
      runSuccess: '检测完成',
      runFailed: '检测失败',
      duplicate: '复制',
      duplicating: '复制中',
      ...
      form: {
        ...
        apiModeModelsHint: '只请求 /v1/models，验证 Key 和模型是否存在，不会产生内容。',
        apiModeImages: '真实生图',
        apiModeImagesHint: '调用 /v1/images/generations 真实生成 1 张测试图，并缓存最近成功的一张。',
        endpoint: '上游地址',
        sourceMode: '监测来源',
        sourceModeDirect: '直连上游',
        sourceModeInternal: '站内监测用户 Key',
        sourceModeHint: '站内模式通过本机网关调用对应分组，不经过公网，也不计入用户费用。',
        internalKey: '站内监测 Key',
        internalKeyPlaceholder: '选择已配置的站内监测 Key',
      internalKeyHint: '这里仅显示 Key 名称和绑定分组，不会显示明文 Key。',
      ensureInternalKeys: '初始化当前平台的站内监测 Key',
      ensureInternalKeysSuccess: '站内监测 Key 已就绪，新建 {count} 个',
      ensureInternalKeysFailed: '初始化站内监测 Key 失败',
      noMonitoringGroups: '当前平台没有可用于监测的分组',
      internalKeyLoadFailed: '加载站内监测 Key 失败',
        ...
        accountGroup: '账号管理分组',
        accountGroupNone: '不使用账号管理分组',
        accountGroupPlaceholder: '选择与当前平台匹配的账号分组',
        accountGroupHint: '关联后监控使用该分组内的真实账号进行粘性探测；静态上游配置保留为兼容回退。',
        providerAntigravity: 'Antigravity',
        intervalSeconds: '检测间隔 (秒)',
        intervalSecondsHint: '范围：15 - 3600 秒；真实生图新建默认 1800 秒（30 分钟）。',
        imageRequestTimeoutSeconds: '生图等待时间 (秒)',
        imageRequestTimeoutSecondsHint: '单次真实生图检测最长等待时间，范围：15 - 900 秒；默认 300 秒（5 分钟）。',
      ...
      runResultTitle: '检测结果',
      forceFullRun: '强制全量',
      forceFullRunTitle: '忽略粘性账号，对当前模型的候选账号运行一次全量检测',
      probeAccount: '账号：{name}',
      bestAccountEmpty: '未记录',
      probeMode: '方式：{mode}',
      probeCandidates: '正常 {healthy} / 候选 {candidates}',
      probeCheckedAt: '检测时间：{time}',
      probeModes: {
        static: '静态',
        sticky: '粘性',
        confirm: '确认',
